...
class FianetSocket {
  ...
  function connect($header) {
    ...
    if ($this->is_ssl)
      $socket = fsockopen('ssl://' . $this->host, $this->port, $this->errno, $this->errstr, FianetSocket::TIMEOUT);
    else
    ...
    if ($socket !== false) {
      ...
      //if header sending is impossible : log
        ...
        insertLog(__METHOD__ . ' : ' . __LINE__, "Envoi des données impossible sur : " . $host);
        $res = false;
      }
      //closes the connexion
      fclose($socket);
    } else { //if connection failed, log
      insertLog(__METHOD__ . ' : ' . __LINE__, "Connexion socket impossible sur l'hôte $host. Erreur " . $this->errno . " : " . $this->errstr);
      $res = false;
    }
